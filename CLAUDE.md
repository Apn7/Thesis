...
Replacing the ESP32 cane hardware with a **Raspberry Pi Zero 2 W + Arducam IMX519** camera that streams JPEG frames to the phone to feed the **same** YOLO pipeline. Validated design (2026-07-04, replaces the earlier BLE/LocalOnlyHotspot plan): the **Pi hosts its own AP** (`SmartCane-Cam`, fallback when no known WiFi at boot; dev hotspot keeps priority) and the app **auto-joins it via `WifiNetworkSpecifier`** (`pi_wifi_service.dart` + `pi_wifi` MethodChannel) — an app-scoped local-only link, so internet (Groq/geocoding) stays on mobile data. **WiFi carries latest-frame-wins JPEG + sonar** (Pi discovers the phone's IP and dials it), and frames go through the plugin's **`YOLO.predict(Uint8List)`** still-image API (since `YOLOView` has no external-frame input). New code is gated behind `enablePiVision`/`enablePiAutoJoin` flags and a parallel screen — **leave `vision_demo_screen.dart` untouched**. Handoff doc: `../../Thesis_pi_zero/STEP2_PROVISIONING_HANDOFF.md`; research: `../../Thesis_pi_zero/PI_ZERO_VISION_PLAN.md`.
