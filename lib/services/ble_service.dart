...
class BleService extends ChangeNotifier {
  ...
  Future<void> startScanning() async {
    ...
    try {
        ...
        (results) {
          ...
          for (ScanResult r in results) {
            ...
            if (name.isNotEmpty || advName.isNotEmpty) {
              debugPrint(
                '>> BLE: Found: platformName="$name", advName="$advName", id=${r.device.remoteId}, rssi=${r.rssi}',
              );
            }

            // Match by platformName OR advertisementData.advName
            final matchName = name.isNotEmpty ? name : advName;
            if (matchName.toLowerCase().contains(
              AppConstants.bleDeviceName.toLowerCase(),
            )) {
              debugPrint(
                '>> BLE: ★★★ SmartCane FOUND! Stopping scan and connecting...',
              );
      ...
      if (_state == BleConnectionState.scanning) {
        ...
        if (_autoReconnect) {
          debugPrint(
            '>> BLE: Will auto-retry in ${AppConstants.bleReconnectDelay.inSeconds}s',
          );
  ...
  Future<void> _discoverAndSubscribe(BluetoothDevice device) async {
    ...
    try {
      ...
      for (var service in services) {
        debugPrint(
          '>> BLE:   Service: ${service.uuid} (${service.characteristics.length} characteristics)',
        );
        for (var char in service.characteristics) {
          debugPrint(
            '>> BLE:     Char: ${char.uuid} [read=${char.properties.read}, notify=${char.properties.notify}, indicate=${char.properties.indicate}]',
          );
        }
      }
      ...
      if (caneService == null) {
        ...
        for (var service in services) {
          if (service.uuid.toString().toLowerCase() == targetStr ||
            ...
            caneService = service;
            debugPrint(
              '>> BLE: ✓ Smart Cane service found via string match: ${service.uuid}',
            );
            break;
          }
        }
      ...
      if (caneService == null) {
        debugPrint(
          '>> BLE: ✗ Smart Cane service NOT FOUND among ${services.length} services',
        );
        _updateState(
          BleConnectionState.error,
      ...
      if (alertChar == null) {
        ...
        for (var char in caneService.characteristics) {
          if (char.uuid.toString().toLowerCase() == targetStr ||
            ...
            alertChar = char;
            debugPrint(
              '>> BLE: ✓ Alert characteristic found via string match: ${char.uuid}',
            );
            break;
          }
        }
      ...
      debugPrint(
        '>> BLE: Char properties: read=${alertChar.properties.read}, notify=${alertChar.properties.notify}, indicate=${alertChar.properties.indicate}',
      );
      ...
      final subscription = alertChar.onValueReceived.listen((value) {
        ...
        debugPrint(
          '>> BLE: ★ ALERT RECEIVED: "$message" (${value.length} bytes)',
        );
      ...
      debugPrint(
        '>> BLE: ✓ setNotifyValue(true) DONE — Pi should show StartNotify',
      );
  ...
  void _scheduleReconnect() {
    if (_autoReconnect) {
      debugPrint(
        '>> BLE: Will retry scan in ${AppConstants.bleReconnectDelay.inSeconds}s',
      );
      Future.delayed(AppConstants.bleReconnectDelay, () {
        if (_state == BleConnectionState.disconnected ||
